        ...
        //Door Option
            ...
            if (!doorChecked) {
                ...
                if (choiceDoor == "yes" || choiceDoor == "y") {
                    ...
                    std::this_thread::sleep_for(std::chrono::seconds(1));
                    std::cout << "\n" <<player.getDisplayName() << " backs up and charges at the door with all their might...\n";
                    std::this_thread::sleep_for(std::chrono::seconds(1));
...
void runProfessionSelection(Character& player, int initialDamage) {
    // Dialogue 1
    std::cout << "\n\nYou hear footsteps approaching your cell... knightly footsteps.\n";
    ...
    while (true) {
        ...
        default:
            std::cout << "\n" << roland.getDisplayName() << ": \"Pathetic. Speak clearly, which of these belongs to you.\"\n";
            continue;
        }
        
        std::cout << "\n" << roland.getDisplayName() << ": \"A " << chosenClass << "? I knew it. Now confirm.\"\n";
    ...
    player.applyInstantDamage(7, core::DamageType::Physical, roland.getDisplayName() + "'s iron fist\n\n");

    std::cout << "Stars burst in your vision. Then darkness.\n";
    std::cout << "You collapse, unconscious... again.\n";
    waitForEnter();
    std::cout << "\n\n" << player.getDisplayName() << "'s Stats: ";
    // Print Stats
    player.printStats();

    // Save Character?
    std::string choice = getInput("=====================\n\nWould you like to save your character? (yes/no): ", true);
